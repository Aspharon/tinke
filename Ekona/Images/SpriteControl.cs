    ...
    public partial class SpriteControl : UserControl
    {
        ...
        private void Export_Single()
        {
            ...
            if (o.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    Image tempImg = sprite.Get_Image(image, palette, comboBank.SelectedIndex, 512, 256, checkGrid.Checked, checkCellBorder.Checked,
                        checkNumber.Checked, false, checkImage.Checked);
                    Tuple<int, int> size = Actions.Get_Dimensions(new Bitmap(tempImg));
                    int width = size.Item1;
                    int height = size.Item2;
                    img = sprite.Get_Image(image, palette, comboBank.SelectedIndex, width, height, checkGrid.Checked, checkCellBorder.Checked,
        ...
        private void Export_All()
        {
            ...
            for (int i = 0; i < sprite.NumBanks; i++)
            {
                ...
                if (checkCrop.Checked)
                {
                    Image tempImg = sprite.Get_Image(image, palette, i, 512, 256, checkGrid.Checked, checkCellBorder.Checked,
                        checkNumber.Checked, false, checkImage.Checked);
                    Tuple<int, int> size = Actions.Get_Dimensions(new Bitmap(tempImg));
                    width = size.Item1;
                    height = size.Item2;
                }
                Image img = sprite.Get_Image(image, palette, i, width, height, checkGrid.Checked, checkCellBorder.Checked,
                    checkNumber.Checked, checkTransparency.Checked, checkImage.Checked);
